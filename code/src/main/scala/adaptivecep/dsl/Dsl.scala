 ...
 // def tumblingWindow (instances: Instances): Window = TumblingInstances (instances.i)
  ...
  case class MessageOverheadHelper2(operator: Operator, load: Int) {
    def otherwise(otherwise: Option[MessageOverheadMeasurement]): MessageOverheadRequirement =
      MessageOverheadRequirement(operator, load, otherwise)
  }
  ...
  abstract class FrequencyMeasurement(){
  ...
  case class Query2Helper[A, B](q: Query2[A, B]) {
    def where                (                              cond: (List[A], List[B]) => Boolean, requirements: Requirement*): Query2[A, B] =                                                                               Filter2       (q, toFunEventBoolean(cond), requirements.toSet)
