...
import adaptivecep.dsl.Dsl.{Frequency, FrequencyMeasurement, LatencyMeasurement, LoadMeasurement, MessageOverheadMeasurement}
...
//  case class TumblingInstances (instances: Int) extends Window
  ...
  case class LatencyRequirement   (operator: Operator, latency: Duration, otherwise: Option[LatencyMeasurement])      extends Requirement
  case class FrequencyRequirement (operator: Operator, frequency: Frequency, otherwise: Option[FrequencyMeasurement]) extends Requirement
  case class LoadRequirement      (operator: Operator, machineLoad: MachineLoad, otherwise: Option[LoadMeasurement])            extends Requirement
  case class MessageOverheadRequirement (operator: Operator, requirement: Int, otherwise: Option[MessageOverheadMeasurement])            extends Requirement
