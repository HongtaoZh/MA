...
case class JoinNode( mode: Mode,
  ...
  def createDuplicateNode(address: Address): ActorRef = {
   NodeFactory.createJoinNode(mode, query, parentNode1, parentNode2, createdCallback, eventCallback, address, context)
   }

  def getParentNodes: Seq[ActorRef] = Seq(parentNode1, parentNode2)
