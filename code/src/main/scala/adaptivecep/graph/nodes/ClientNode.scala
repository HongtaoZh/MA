...
case class ClientNode(operatorNode: ActorRef, monitorFactories: Array[MonitorFactory]) extends ClusterActor with ActorLogging {
  ...
  override def receive: Receive = {
    case TransitionRequest(algorithm) => {
      ...
      operatorNode ! TransitionRequest(algorithm)
    }
  ...
  override def preStart(): Unit = {
    super.preStart()
    log.info(s"Subscribing for events from ${operatorNode.path.name}")
    operatorNode ! Subscribe()
  }
}