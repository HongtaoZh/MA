...
/**
  * Handling of [[adaptivecep.data.Queries.DropElemQuery]] is done by DropElemNode.
  *
  * @see [[QueryGraph]]
  * */
...
case class DropElemNode( mode: Mode.Mode,
  ...
  override def childNodeReceive: Receive = super.childNodeReceive orElse {
    case DependenciesRequest =>
      sender ! DependenciesResponse(Seq(parentNode))
    case Created if sender().equals(parentNode) =>
      emitCreated()
  ...
  def createDuplicateNode(address: Address): ActorRef = {
    NodeFactory.createDropElementNode(mode, query, parentNode, createdCallback, eventCallback, address, context)
  }
