...
/**
  ...
  */
class BenchmarkingNode extends ClusterActor with ActorLogging {
  ...
  def selectBestPlacementAlgorithm(requirements: List[Requirement]): PlacementAlgorithm = {
    ...
    for (requirement <- requirements) {
      requirement match {
        case LatencyRequirement(_, _, _, _) => filterAlgos("latency")
        case LoadRequirement(_, _, _,_) => filterAlgos("machineLoad")
        case MessageOverheadRequirement(_, _, _,_) => filterAlgos("messageOverhead")
      }
    }
...
case class PlacementAlgorithm(algorithm: PlacementStrategy, requirements: List[NetworkProperties],
  ...
  def containsDemand(demand: Requirement): Boolean = {
    demand match {
      case LatencyRequirement(_, _, _, _) => demands.contains("latency")
      case LoadRequirement(_, _, _,_) => demands.contains("machineLoad")
      case MessageOverheadRequirement(_, _, _,_) => demands.contains("messageOverhead")
    }
  }
}
