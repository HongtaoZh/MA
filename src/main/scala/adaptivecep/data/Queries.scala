...
import adaptivecep.dsl.Dsl.{LatencyMeasurement, LoadMeasurement, MessageOverheadMeasurement}
...
//  case class TumblingInstances (instances: Int) extends Window
  ...
  case class LatencyRequirement   (operator: Operator, duration: Duration, then:Option[LatencyMeasurement], otherwise: Option[LatencyMeasurement])      extends Requirement
  case class FrequencyRequirement (operator: Operator, instances: Int, seconds: Int, callback: Int => Any) extends Requirement
  case class LoadRequirement      (operator: Operator, machineLoad: MachineLoad, then:Option[LoadMeasurement], otherwise: Option[LoadMeasurement])            extends Requirement
  case class MessageOverheadRequirement (operator: Operator, requirement: Int, then:Option[MessageOverheadMeasurement], otherwise: Option[MessageOverheadMeasurement])            extends Requirement
