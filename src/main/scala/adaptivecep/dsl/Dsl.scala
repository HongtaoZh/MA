...
/**
  ...
  * [[adaptivecep.data.Events]] & [[adaptivecep.data.Queries]]
  * */
...
object Dsl {
 ...
 // def tumblingWindow (instances: Instances): Window = TumblingInstances (instances.i)
  ...
  case class FrequencyHelper2(operator: Operator, ratio: Ratio) {
    def otherwise(callback: Int => Any): FrequencyRequirement =
  ...
  def overhead: MessageOverheadHelper.type = MessageOverheadHelper

  case object MessageOverheadHelper {
    def === (overhead: Int): MessageOverheadHelper2 = MessageOverheadHelper2 (Equal, overhead)
    def =!= (overhead: Int): MessageOverheadHelper2 = MessageOverheadHelper2 (NotEqual, overhead)
    def >   (overhead: Int): MessageOverheadHelper2 = MessageOverheadHelper2 (Greater, overhead)
    def >=  (overhead: Int): MessageOverheadHelper2 = MessageOverheadHelper2 (GreaterEqual, overhead)
    def <   (overhead: Int): MessageOverheadHelper2 = MessageOverheadHelper2 (Smaller, overhead)
    def <=  (overhead: Int): MessageOverheadHelper2 = MessageOverheadHelper2 (SmallerEqual, overhead)
  }
  ...
  abstract class MessageOverheadMeasurement(){
    def apply(overHead: Int): Any
  }

  case class MessageOverheadHelper2(operator: Operator, load: Int) {
    def then(then: Option[MessageOverheadMeasurement]): MessageOverheadHelper3 =
      MessageOverheadHelper3(operator, load, then)
  }

  case class MessageOverheadHelper3(operator: Operator, load: Int, then: Option[MessageOverheadMeasurement]) {
    def otherwise(otherwise: Option[MessageOverheadMeasurement]): MessageOverheadRequirement =
      MessageOverheadRequirement(operator, load, then, otherwise)
  }

  abstract class LatencyMeasurement(){
    def apply(latency: Duration): Any
  }

  case class LatencyHelper2(operator: Operator, duration: Duration) {
    def then(then: Option[LatencyMeasurement]): LatencyHelper3 =
      LatencyHelper3(operator, duration, then)
  }


  case class LatencyHelper3(operator: Operator, duration: Duration, then: Option[LatencyMeasurement]) {
    def otherwise(otherwise: Option[LatencyMeasurement]): LatencyRequirement =
      LatencyRequirement(operator, duration, then, otherwise)
  }

  abstract class LoadMeasurement(){
    def apply(load: MachineLoad):Any
  }

  case class LoadHelper2(operator: Operator, load: MachineLoad) {
    def then(then: Option[LoadMeasurement]): LoadHelper3 =
      LoadHelper3(operator, load, then)
  }

  case class LoadHelper3(operator: Operator, load: MachineLoad, then: Option[LoadMeasurement]) {
    def otherwise(otherwise: Option[LoadMeasurement]): LoadRequirement =
      LoadRequirement(operator, load, then, otherwise)
  }
