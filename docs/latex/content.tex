...
\includegraphics[scale=0.9]{FMexample.png}
\end{figure}
...
To generate a feature model of a \gls{spl} it is necessary to perform a \textit{domain analysis} of the domain to be modeled. The term domain describes the set of current and future applications which share a set of common capabilities and data \cite{Kang1990}. In \cite{Kang1990}, Domain analysis serves to identify and represent the information relevant to a domain from various sources and  encompasses three steps: First, analysis of the domain context to define its extent, second, Domain modeling describing the problems addressed by the software within the defined domain, and third, the creation of software architectures that solve problem in the domain. 
The feature model is created as part of the Feature Analysis during Domain modeling phase. Feature analysis serves to understand the general capabilities and different applications of a domain in order to be able to identify the system properties that directly affect the end-user. Identified features are then abstracted, classified and structured in a feature model.
%in which every relevant system property is identified and mapped to a feature.
...
%The task of identifying and organizing commonalities and variabilities in the form of shared data and capabilities in a domain is called feature analysis?
...
% mention of papers for related work here as it is best explained in context of other work and is directly relevant to the thesis \\ \\
