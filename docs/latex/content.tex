...
%TODO rename to DSP (Data Stream Processing)? - used more commonly in papers OR note that sometimes referred to as DSP or ESP
\% EBS basics
...
%EBS: definition of.... events, producers, consumers

Complex Event Processing (CEP) is a term used for a collection of methods and tools that allow processing of event streams in a timely manner. The goal is to extract higher-level information from a stream of lower-level events by declaratively specifying events of interest through a query language. 


%event definition
An \it{event} can be defined as anything that happens \cite{Chandy2011}, or more precisely, any meaningful change of state in the universe that can be perceived. Depending on the application, a mere change in time can be an event, so an ordered sequence of data generated by a source like a light sensor can be interpreted as an \it{event stream}. To be precise, when we use the term \it{event} we actually mean a \it{notification about an event}, not the event itself that caused the change of state. This \it{event notification} is a message that contains one or more attribute-value fields and usually has a timestamp and a type.
...
This declarative approach to retrieving information is similar to traditional Database Managment Systems (DBMS), hence the Query languages used in CEP and Data Stream Processing (DSP) in general are structurally very similar. 
%TODO possibly give example of SQL vs CQL
...
Furthermore aggregation operators can only work if some form of windowing is applied to the stream (e.g. a sliding window of the last 10 event instances).

%types of operators in more detail
Queries - sometimes also referred to as event patterns \cite{Luckham2011a} or event definition rules \cite{Cugola2013} - consist of one or more operators. They can be categorized as follows:
\begin{itemize}
\item Aggregation: operations that work on a finite set of values (i.e. windowed events), like count, sum, average, median, minimum, maximum
...
The CEP system is responsible for the processing of incoming events and routing detected complex events to any interested consumers.
